// Service: Reject doctor verification
// POST /api/v1/admin/doctor-verifications/{doctorId}/reject?adminId=...&remarks=...
 ...
 */
export async function rejectDoctorVerification({ doctorId, adminId, remarks }) {
  if (doctorId == null) throw new Error('Thiếu doctorId');
  if (adminId == null) throw new Error('Thiếu adminId');
  if (!remarks) throw new Error('Thiếu lý do (remarks)');

  const url = `${BASE_URL}/api/v1/admin/doctor-verifications/${encodeURIComponent(String(doctorId))}/reject`;
  ...
  if (!res.ok) {
    const message = data?.message || `Từ chối hồ sơ thất bại (${res.status})`;
    throw new Error(message);
  }
  return data;
}

export default rejectDoctorVerification;
