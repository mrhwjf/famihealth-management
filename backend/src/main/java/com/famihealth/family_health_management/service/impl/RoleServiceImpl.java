...
public class RoleServiceImpl implements RoleService {
	...
	public RoleDetailDto create(RoleCreateRequest req) {
		...
		if (req.getPermissionIds() != null) {
			role.setPermissions(fetchPermissionsByIds(req.getPermissionIds()));
		}
	...
	private java.util.Set<Permission> fetchPermissionsByIds(java.util.Set<Integer> ids) {
		if (ids.isEmpty()) {
			return new java.util.HashSet<>();
		}
		java.util.Set<Integer> distinct = new java.util.HashSet<>(ids);
		java.util.List<Permission> found = permissionRepository.findAllById(distinct);
		if (found.size() != distinct.size()) {
			throw new ResourceNotFoundException("One or more permissions not found for IDs=" + distinct);
		}
